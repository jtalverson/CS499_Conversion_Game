...
public class PaperStack : MonoBehaviour
{
    ...
    void Start()
    {
        pageHolder = GameObject.Find("PaperStack").transform;
        currentPage = pageHolder.GetChild(0);
        // Set start position to current position
        transformStart = currentPage.position;
        // Set starting rotation
        rotationStart = currentPage.rotation;
    }
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0 && !reset)
        {
            ...
            switch (touch.phase)
            {
                ...
                case TouchPhase.Moved:
                    ...
                    currentPage.position -= new Vector3(travelDistance, 0);

                    if (currentPage.name.Contains("RX"))
                    {
                        currentPage.RotateAround(currentPage.position, Vector3.forward, -rotationAngle * travelDistance);
                    }
                    stepsTaken += 1;
        ...
        if (reset)
        {
            ...
            if (overallDistance > 0)
            {
                currentPage.RotateAround(currentPage.position, Vector3.forward, rotationAngle * resetStep);
                currentPage.position += new Vector3(resetStep, 0);
            }
            else
            {
                currentPage.RotateAround(currentPage.position, Vector3.forward, -rotationAngle * resetStep);
            ...
            if (Mathf.Abs(currentPage.position.x - transformStart.x) <= resetStep)
            {
                currentPage.position = transformStart; // Reset position
                currentPage.rotation = rotationStart; // Reset orientation
                reset = false;
            }
        }

        /*if (Mathf.Abs(overallDistance) >= swipeDistance && overallDistance < 0)
        {
            Utility.FindObject(currentPage.gameObject, "Rejected").gameObject.SetActive(true);
        }
        else if (Mathf.Abs(overallDistance) >= swipeDistance && overallDistance > 0)
        {
            Utility.FindObject(currentPage.gameObject, "Rejected").gameObject.SetActive(true);
        }*/
    }
    ...
    private IEnumerator FlingAndReturn(bool leftORright)
    {
        ...
        if (leftORright)
        {
            ...
            while (currentPage.position.x > -endPosition)
            {
                // Update position
                currentPage.position -= new Vector3(stepSize, 0);
                // Pause for a frame
                yield return null;
            }
        }
        ...
        yield return new WaitUntil(() => currentPage != null);
        if (pageHolder.childCount > 0)
            currentPage = pageHolder.GetChild(0);
    }
}
